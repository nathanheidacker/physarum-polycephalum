...
; A slightly more elaborate setup that demonstrates the multinucleate
...
to propagate-signal
  ...
  if carrying-signal > 0 [
    ...
    ask other cytoplasm-here [
      ...
      ; This does not violate conservation of matter; the fluid particle should not be conveived of as inheriting
      ; some quantity of matter, but rather something akin to being tagged by a trace chemical agent
      ; with a very specific lifetime / halflife.
...
to place-nucleus
  ...
  ; multinucleate organisms, so multiple nuclei can exist simultaneously
...
P. Polycephalum uses these basic mechanisms to demonstrate complex behavior, such as navigating mazes to find the shortest route between food sources, and using said pathfinding ability to reconstruct transportation maps of different cities (Alim). 

Large cities typically aim to optimize public transportation routes by reducing path length between stations, saving the city money the the passengers their time. The ability to recreate these public transport maps when given the station locations, then, should be indicative of some kind of pathfinding ability.

When food is laid out on a flat surface with the same relative positions as the public transport stations in downtown Tokyo, Physarum Polycephalum is able to construct a plasmodic network with strongest paths that are remarkably similar in shape and structure to the actual transportation maps, indicating that it has found optimized path lengths (Alim). {{{ cities? What the heck is a slime mold doing reconstructing transportation maps of ctties? }}} (I added this explanation here to address this comment, but honestly after reading it im not sure it fits with the rest of the content in this section)
...
At every tick, each fluid agent will perform the following actions in order:
 ...
 - If carrying a food signal, other fluid agents that occupy the same patch in this tick will receive a food signal of the same value less one. If another fluid agent in the same patch is carrying a food signal with a higher value, they will not inherit the food signal of lower value.
...
By switching on visualize-velocty?, one can directly see the velocity of fluid agents in the network via color coding. Particles moving quickly are represented in red, while particles moving slowly are represented in green, with intermediate values being represented by shades inbetween.


## HOW TO USE IT

To begin, press the setup button which will intialize the world-state. This button can be pressed again at any time to reset everything in the environment. To create the environment, use the draw function to place P. Polycephalum nuclei, walls, and food. Four distinct setup environments exist to demonstrate different aspects of the model:
 ...
 - Shortest-path: A world that demonstrates the ablity of P Polycephalum to select the shortest-path 
...
Kill Slime-Mold: Kills all agents. Allows the user to 'reset' while preserving the custom environment.
...
Notice how the strength of any given tube (its size) is proportional to its velocity. Switching between visualization modes is a great way to get an intuitive understanding of this mechanism. Notice how the weaker fringes of the network will appear green, and hotspots will appear red. In the standard visualization, these velocity differences will be reflected in the size of the respective tubes.

When placing nutritive stimuli (especially permanent stimuli with eat-proportion = 0), notice how the network will adapt to be significantly more developed around the area of the food source, both in the strength of the tubes surrounding the food and the number of branching paths. Use visualize-signaling-molecule? to gain more precise intuitions about how this works.
